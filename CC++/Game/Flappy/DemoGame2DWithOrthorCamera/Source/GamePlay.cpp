...
void GamePlayState::Update(float deltatime)
{
	...
	for (int i=0;i<6;i++)
		...
		if (birdIsFly)
		{
			...
			birdpos.y = y0 + 5.0f*eslapedTime - 0.5*9.8*eslapedTime*eslapedTime;
...
void GamePlayState::RotateBird(float angle)
{
	if (angle==-3.14f/2.0f)
	{
